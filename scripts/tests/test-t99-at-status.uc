...
import { parse_iccid, t99_at_status } from '../../package/vtmodem/files/usr/share/vtmodem/t99-radio.uc';
...
let replies = {
	...
	'AT+ICCID': 'ICCID: 8900000000000000001F\n'
};
...
assert(length(calls) == 4, 'Four read queries');
assert(result.t99_iccid === '8900000000000000001', 'ICCID padding removed without numeric conversion');
...
assert(parse_iccid('AT+ICCID\r\nICCID: 0000000000000000001f\r\nOK\r\n') === '0000000000000000001', 'Keep leading zeroes and strip only final filler');
assert(parse_iccid('ICCID: 89000000000000000010') === '89000000000000000010', 'Preserve all twenty decimal digits');
assert(parse_iccid('ICCID: 8900000000000000001') === '8900000000000000001', 'Unpadded identifier');
for (let bad in [ 'ERROR', 'ICCID: 8900F00000000000001', 'ICCID: 8900000000000000001FF', 'ICCID: 123', '8900000000000000001' ])
	assert(parse_iccid(bad) == null, 'Reject malformed or unlabelled identifiers');
result = t99_at_status('/dev/t99w175-at', function(argv, timeout_ms) {
	if (argv[4] == 'AT+ICCID')
		return { ok: false, output: replies[argv[4]] };
	return success(argv, timeout_ms);
});
assert(result.t99_iccid == null && result.t99_temperature.tsens_c == 29, 'Failed ICCID query preserves radio data');
